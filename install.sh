    ...
    if [ "$DRY_RUN" = false ]; then
        if nslookup $DOMAIN &>/dev/null; then
            ...
            if certbot --nginx -d $DOMAIN --non-interactive --agree-tos --email admin@$DOMAIN --redirect --quiet; then
                ...
                # Setup auto-renewal
                cat > "/etc/cron.d/certbot-$APP_NAME" << CERTBOT_CRON_EOF
...
FAIL2BAN_EOF
    ...
    cat > "/etc/fail2ban/filter.d/nginx-req-limit.conf" << 'NGINX_FILTER_EOF'
[Definition]
failregex = limiting requests, excess: .* by zone .*, client: <HOST>
ignoreregex =
NGINX_FILTER_EOF
    
    cat > "/etc/fail2ban/filter.d/gym-tracker-api.conf" << 'API_FILTER_EOF'
[Definition]
...
CRON_BACKUP_EOF
    ...
    cat > "$APP_DIR/scripts/verify-backup.sh" << 'VERIFY_BACKUP_EOF'
#!/bin/bash
...
if [ $AGE_HOURS -gt 30 ]; then
    echo "Warning: Latest backup is older than 30 hours"
fi
...
optimize_system() {
    ...
    cat > "$APP_DIR/scripts/optimize-db.js" << 'OPTIMIZE_DB_EOF'
...
`, function(err) {
    if (err) {
        ...
        // Show database stats
        db.get("SELECT COUNT(*) as users FROM users", function(err, result) {
            if (!err) console.log('Users:', result.users);
        });
        
        db.get("SELECT COUNT(*) as workouts FROM workouts", function(err, result) {
            if (!err) console.log('Workouts:', result.workouts);
        });
        
        db.get("SELECT COUNT(*) as exercises FROM exercises", function(err, result) {
...
OPTIMIZE_DB_EOF
    ...
    cat >> "/etc/cron.d/$APP_NAME-monitor" << 'OPTIMIZE_CRON_EOF'
...
download_application() {
    ...
    if [ -d ".git" ]; then
        ...
        # Check if directory has files (excluding our standard directories)
        ...
        # Move contents from temp directory to app directory
...
MONITORSH_EOF
    ...
    cat > "$APP_DIR/scripts/update.sh" << 'UPDATESH_EOF'
#!/bin/bash
