...
download_application() {
    ...
    elif [ "$(ls -A $APP_DIR 2>/dev/null | grep -v -E '^(public|database|scripts|logs|backups|uploads)
...
main "$@" | wc -l)" -gt 0 ]; then
        print_sub "Directory not empty, backing up existing files..."
        execute_command mkdir -p "$APP_DIR/backup-$(date +%Y%m%d-%H%M%S)"
        execute_command find $APP_DIR -maxdepth 1 -type f -exec mv {} "$APP_DIR/backup-$(date +%Y%m%d-%H%M%S)/" \;
        
        print_sub "Initializing git repository..."
        execute_command git init
        execute_command git remote add origin $GITHUB_REPO
        execute_command git fetch origin
        execute_command git checkout -b main origin/main
    else
        print_sub "Cloning from GitHub..."
        # Remove the directories we just created since git clone needs empty directory
        execute_command rm -rf {public,database,scripts,logs,backups,uploads} 2>/dev/null || true
        execute_command git clone $GITHUB_REPO .
        
        # Recreate directories if they don't exist after clone
        execute_command mkdir -p {public/{js,css},database,scripts,logs,backups,uploads}
    fi
